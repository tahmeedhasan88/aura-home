...
const getHomePosition = (home) => {
  if (home?.coordinates?.lat && home?.coordinates?.lng) {
    return [Number(home.coordinates.lat), Number(home.coordinates.lng)];
  }

  const location = typeof home?.location === "string" ? home.location : "";
...
const MapContent = dynamic(
    ...
    return function LeafletMap({ homes }) {
      return (
        ...
        >
          ...
          {homes.map((home) => (
            <Marker key={home.id} position={getHomePosition(home)} icon={markerIcon}>
              <Popup>
                <div className="space-y-1">
                  <h3 className="font-semibold">{home.name}</h3>
                  <p className="text-sm text-gray-600">{home.location}</p>
                  <p className="font-medium">{home.price}</p>
                </div>
              </Popup>
            </Marker>
          ))}
  ...
  },
  {
    ssr: false,
    loading: () => (
      <div className="flex h-[500px] items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 text-sm text-slate-500 md:h-[650px]">
        Loading map…
      </div>
    ),
  }
...
export default function MapView() {
  ...
  return (
    ...
    <section className="mx-auto w-full max-w-7xl pt-10 px-4 py-8">
